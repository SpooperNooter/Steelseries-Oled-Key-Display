...
from UtilityModules import Bitmap, Sprite
import time
import ctypes
import math
...
class Afk:
    ...
    def __init__(self, CollectionPath):
        ...
        self.Startup = Sprite(self.Transition,self.StartAnimation)
        self.Rp = Sprite(self.RepeatAnimation, RepeatOnFinished=True)
        self.End = Sprite(self.EndingTransition)
...
class Keystroke_logging:
    ...
    def __init__(self, CollectionPath):
        ...
        self.Collection = CollectionPath
        self.Base = Bitmap(FromPng=f"Animations/{self.KeyboardSet}.png")
        self.Background = Bitmap(128,40)
        self.MouseOverlay = Bitmap(128,40,2)
        self.WPM = 0
        ...
        self.KeyDictionary = {i:(self.Keydata(i, a[i], Sprite(f"Animations/{self.KeyboardSet}/Button")) if i != "Key.space" else self.Keydata(i, a[i], Sprite(f"Animations/{self.KeyboardSet}/Spacebar"))) for i in a}
        self.CtrlDictionary = {"\x11" : "Q", "\x17": "W", "\x05" : "E", "\x12" : "R", "\x14": "T", "\x19" : "Y",
                               ...
                               "\x0e" : "N", "\r" : "M"}
        self.KeyDictionary["Button.left"] = self.Keydata("Button.left",(0,0), Sprite("Animations/LeftClick"),3)
        self.KeyDictionary["Button.right"] = self.Keydata("Button.right",(122,0), Sprite("Animations/RightClick"),3)
        ...
        self.VolumeTransition = Sprite("Animations/VolumeTransition")
        self.VolumeBuddy = Sprite("Animations/VolumeBuddy", RepeatOnFinished=True, RepeatIndex=77)
        self.VolumeBar = Sprite("Animations/VolumeBar")
        self.VolumeTime = 0
    ...
    def Return_base(self):
        ...
        for i in self.KeyDictionary.values():
            ...
            if i.Mode == "release":
                ...
                if i.Sprite.FrameOn > i.DivideFrame:
                    ...
                    a = i.Sprite.next()
            if a != None:
                self.Base.AlterBitmap(a, i.Offset)
        ...
        if (p := self.volume.GetMasterVolumeLevelScalar() * 100) != self.PastVolumeLevel or self.VolumeTime != 5*self.Collection.FPS and self.VolumeTime != 0:
            ...
            if not self.VolumeTransition.SpriteNumber >= 14:
                self.VolumeTime = 1
                C = c.Copy()
                C.AlterBitmap(self.VolumeTransition.next(), Invert=True,FullAlter=True)
                h = self.Base.Copy()
                h.AlterBitmap(C)
                return h
            else:
                C = c.Copy()
                C.AlterBitmap(self.VolumeTransition.seek(self.VolumeTransition.FrameOn), Invert=True, FullAlter=True)
            ...
            if self.VolumeBar.FrameOn != 0:
                C = c.Copy()
                C.AlterBitmap(self.VolumeTransition.seek(self.VolumeTransition.FrameOn), Invert=True, FullAlter=True)
                C.AlterBitmap(self.VolumeBar.next(reverse=True), (44, 15))
                return C
            if self.VolumeBar.FrameOn == 0:
                l = c.Copy()
                l.AlterBitmap(self.VolumeTransition.next(), Invert=True, FullAlter=True)
                L = self.Base.Copy()
                L.AlterBitmap(l)
                return L
...
class TestModule:
    ...
    def __init__(self, CollectionPath):
        self.Base = Bitmap(128,40,1)
        self.Base.AlterBitmap(Bitmap(10,10,1),(random.randint(0,100),random.randint(0,100)), Invert=True)
        self.Collection = CollectionPath
